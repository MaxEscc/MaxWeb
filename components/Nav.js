...
const Nav = () => {
  ...
  bottom-0 mt-auto xl:right-[2%] z-50 top-50 w-full xl:w-16 xl:max-w-md xl:h-screen'>
    {/* inner */}
    <div className='flex w-full xl:flex-col items-center justify-between
     xl_justify-center gap-y-10 px-4 md:px-40 xl:px-0 h-[80px] xl:h-max py-8 bg-white/10 
      ...
      {navData.map((link, index) => { 
        return(
         <Link className={`${link.path === pathname && 'text-accent'}
          relative flex items-center group hover:text-accent transition-all duration-0`}
         href={link.path}
         key={index}
         >
